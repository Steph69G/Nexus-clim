...
-- Trigger : Auto-création rapport à l'entrée EN_INTERVENTION
...
BEGIN
  -- Seulement si passage à EN_INTERVENTION
  IF NEW.status = 'EN_INTERVENTION' AND (OLD.status IS NULL OR OLD.status != 'EN_INTERVENTION') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM intervention_reports WHERE mission_id = NEW.id) THEN
      -- Trouver template actif pour ce type de mission
      ...
      -- Mettre à jour mission avec snapshot template
...
-- Trigger : Marquer closed_at quand conditions de clôture sont remplies
...
DECLARE
  v_is_closed boolean;
BEGIN
  -- Calculer manuellement si mission est close
  v_is_closed := (
    NEW.status = 'TERMINEE'
  ...
  -- Si closed et pas encore marqué
...
-- =====================================================
-- 7) RLS POLICIES (logs en lecture seule pour concernés)
-- =====================================================
...
-- Techniciens : voir logs de leurs missions
...
-- Invoices : admin/manager full access
